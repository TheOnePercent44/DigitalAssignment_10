...
Lottery.Game.prototype = {
	...
	////Input Handlers/////////////////////////////////////////////////////////////////////////////////	
		...
		{
			canRepulse = false;
			baddies.forEachAlive(repulse, this.game);
		...
		if(!isShooting && (leftKey.isDown))
		{
			player.shoot(bulletgroup, -1, 0);
			isShooting = true;
		}
		else if(!isShooting && (rightKey.isDown))
		{
			player.shoot(bulletgroup, 1, 0);
			isShooting = true;
		}
		else if(!isShooting && (downKey.isDown))
		{
			player.shoot(bulletgroup, 0, 1);
			isShooting = true;
		}
		else if(!isShooting && (upKey.isDown))
		{
			player.shoot(bulletgroup, 0, -1);
			isShooting = true;
		}
		else{}//do nothing
		if(!leftKey.isDown && !rightKey.isDown && !downKey.isDown && !upKey.isDown)
		{
			isShooting = false;
		}
	////Time-set Handlers//////////////////////////////////////////////////////////////////////	
...
{
	if(distance(enemysprite.x, enemysprite.y, player.sprite.x, player.sprite.y) < 150)
	{
